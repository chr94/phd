...
\section{Introdução}
    ...
    %% Comentários Pós-aula Instrumentalização da Tese 2023-03-29 (Gislaine Beloto):
        ...
        % "[Pior que seu trabalho dá para uma teoria, mas não vou falar assim para não te dar bola]. Em cinco a dez anos [meu trabalho/tese] pode chegar a ser uma teoria [após ser trabalhado com outras hipóteses e desdobramentos que confiram confiabilidade à pesquisa], mas na tese [agora] posso dar o primeiro passo para que isso aconteça."
        % "Agora o que temos é a 'definição do conceito'."
        % "A tese dá uma primeira versão ao termo 'morfoadequabilidade' [que não será a última]."
        % "Não posso escancarar isso no problema, pois a 'morfoadequabilidade' é a solução para o problema."
        % "Talvez o problema seja o 'qual o porquê das cidades serem feias?' (cf. Daverio: 'desarmônicas')."
        % "[Na proposta atual/'à la Renato', com manual e diretrizes,] qual a diferença do que eu vou fazer para [o que] uma lei municipal [faz]?"
            % "Na lei se diz para seguir a topografia, os topógrafos sabem e não seguem o relevo; quanto aos percursos, quem disse que são bons? Você tem de provar."
        % "Definição de qualidade:
         %Loteamento (?)
         %Parcelamento (?)
         %Traçado urbano (?)
            % Qual a escala?
        % "'Qualidade', termo amplo e complicado [pois é subjetivo]:"
            % [O que é qualidade para você pode não ser para o empreendedor ou para o usuário, como na questão dos centros e dos polos; quem se importa com isso?]
            % "Quem passou perto desse termo [qualidade] foi a Karin, e raspando."
        % "Essas 'questões' devem embasar o meu problema [de pesquisa]."
        % "Ao me questionar, vou dando [substância] ao meu problema de pesquisa."
        % ~~~
        % "Mas eu induzo extremamente [demais] a resposta ao jogar isso na escola italiana [i.e., a solução do(s) problema(s)].
            % [A saída é]:
                % 1. "Então eu vou ter de provar isso."
                % 2. A menos que eu parta da premissa de que isso é o ideal." [A saber, o que diz a escola italiana sobre traçados e parcelamentos; a premissa será a de que o método dela é o ideal].

    %% Comentários Orientação Renato 2023-04-03 ECI c.a. 11:20
        % No fim, posso usar o termo 'morfoadequabilidade', mas o importante é fazer o projeto [do(s) traçado(s)] e, do projeto, tirar estratégias e diretrizes que serão testadas no ponto focal.
        % Como construção da tese, [fazer] algo mais pragmático.
        % "Usar a topografia para desenvolver um traçado [em específico e em geral] que tenha qualidades formais e estéticas."
            % Ok, mas o que seriam essas 'qualidades formais e estéticas' e em que me respaldar?
                % Unwin (base) [pois, por mais que ele esteja desatualizado (afinal tem 100 anos), não tenha sido traduzido para o português e seja desconhecido, dele surgiram diversos Urban Design Guidelines e Manuals];
                % Urban Design Guidelines atuais (o que está sendo levantado hoje?);
        % No Brasil, não temos instrução de projeto [de novas áreas urbanas], quiçá manual.
        % Unwin não será um ponto de partida em si mesmo, mas o instrumento para analisar Maringá, o traçado de Maringá.
            % No Brasil, não temos manuais de como projetar novas áreas urbanas (loteamentos, bairros, cidades), mas temos uma cidade projetada desde o princípio (ex novo, ex nihil, ex principio) – que é Maringá; assim, vamos tomar partido de Maringá e ver:
                % Qual a base do plano de Maringá? Um plano;
                % Esse plano foi feito por quem? Por um engenheiro – Vieira;
                % O que esse engenheiro seguiu para fazer esse plano? Um saber empírico? Algum manual? Um manual;
                % Esse manual era algo apenas de desenho viário (à la Mascaró)? Ou era algo mais complexo e sistematizado? Era mais complexo e sistematizado;
                % Quem escreveu esse manual? Raymond Unwin;
                % O que tem nesse manual de Unwin? E o que reverberou dele em Maringá? <Aqui começa a minha (nova) leitura de Maringá e de Unwin>;
        % Não se trata apenas de topografia, mas de algo orgânico (no sentido não apenas de consonância com a topografia e suas estruturas, mas que combine tudo);
        % Leitura do traçado de Maringá baseada em Unwin: "usar os acidentes [do relevo] como algo intencional" (Unwin).
        % [HRC] ~Mas, em se tratando de instruções, diretrizes, ou mesmo de um manual, eu não vou me restringir a Maringá; nesse caso, eu terei de lidar com expansões urbanas sobre um parcelamento rural que não é o Waldhufendörf, como ocorre no território da CTNP. Nesse caso, tenho duas alternativas:
            % 1. Falar que basta fazer um land readjustment e pronto, e lavar as mãos;
            % 2. Falar em um projeto geral: pegar o promontório e planejar o traçado inteiro daquela área, de modo que, quando a prefeitura liberar a expansão e o proprietário for lotear a gleba rural, ele já tenha um parcelamento já pronto ao qual deve seguir – proposta que deixou as professoras Karin e Gislaine horrorizadas, posto que isso só aconteceria em uma ditadura (pois a lei atual não permite isso), muito embora meu papel aqui não seja seguir necessariamente a legislação, mas de propor alterações nela; eu sou aquele que arquiteta, que delineia (design) aquilo que vai se tornar lei e não o contrário (e falo aqui como quem se olha no espelho em processo de apoteose: eu estou acima da lei, o meu pensamento está acima da lei vigente – posto que baseado em diversas ideias que não são contempladas pela lei atual). 
            % Nesse caso, minhas diretrizes se aplicam mais ao arquiteto da prefeitura, que vai conceber o conjunto, do que necessariamente ao arquiteto/profissional que o empreendedor contrata para parcelar seus lotes rurais.
                % [RLR] ~[O que você pode fazer é um planejamento por etapas]:
                    % 1. Uma etapa para o arquiteto da prefeitura (com diretrizes viárias, como as do Nildo) <em que eu posso discutir a legislação em vigor>;
                    % 2. [e uma outra etapa em que vou] planejar uma grande área com distintos lotes rurais;
                    % 3. [ou ainda] diretrizes para diferentes áreas que devem se juntar (lógica das 'variações sobre um tema' que discuti com o maestro Thiago Santos);
                % [RLR] *Usar o plano de diretrizes viárias do Nildo (e o projeto do Luis) para fazer a crítica:
                    % [Pense em] "Como eu faria a expansão de Maringá para 2050?"
                        % [Isso] rende:
                            % 1 Capítulo sobre 'projeto numa tacada só' (cf. plano Vieira);
                            % 1 Capítulo com 'projeto faseado'
                                % <Obs. minha: seja faseado por 'layers' (sobrepostos)>;
                                % <Obs. minha: seja faseado com 'pedaços' distintos que se unem (variações sobre um tema):
                                    % a) pedaços que seguem uma mesma lógica (têm uma coerência e uma mesma estruturação, ainda que sejam diferentes);
                                    % b) pedaços que não seguem a mesma estruturação, embora sejam amarrados por uma estrutura comum (o 'tema')>;
                            % 1 Capítulo com diretrizes de projeto;
                            % 1 Capítulo com grupo focal;
                % [RLR] Importante notar que Unwin não dá um método 'receita de bolo'. Não é como um organograma em que vou juntando ponto tal, tal e tal que chegou no conceito tal e no método tal. (Isso se aproximaria de algo platônico). Ao contrário, melhor é partir do projeto e, dele, extrair as características e diretrizes (percurso aristotélico, algo mais ligado à "práxis"). [Segundo ele, o Renato], Trabalhar a partir da formulação de um conceito ou teoria te deixaria muito amarrado caso você precisasse fazer alguma alteração depois, durante a fase de projeto.
                % Sobre o método de pesquisa: veja com qual você mais se aproxima, se EBD ou DSR.
                    % Se eu começar falando do traçado de Maringá, do traçado que eu fiz para Maringá e do traçado do Luis para Maringá, eu estarei fazendo evidência, logo, estarei mais próximo de Evidence-Based Design.
    ...
    \begin{figure}[h]%h=here, t=top of page, p= dedicated page, posso usar `[htp]` para dizer que a figura pode estar em qualquer uma dessas posições e que o Latex deve se encarregar de encontrar a melhor opção.
        \centering
    ...
    \begin{figure}[h]%h=here, t=top of page, p= dedicated page, posso usar `[htp]` para dizer que a figura pode estar em qualquer uma dessas posições e que o Latex deve se encarregar de encontrar a melhor opção.
        \centering
        \includegraphics[width=1\textwidth]{Pictures/nolli_popolo.png}
        \captionsetup{labelfont=bf}
    ...
    \end{figure} 
    %\footnote[3]{Disponível em: \hyperlink {https://www.lib.berkeley.edu/EART/maps/nolli.html}{UC Berkeley Library}. Acesso em: 15 de abril de 2023, 01:22:25.}  
